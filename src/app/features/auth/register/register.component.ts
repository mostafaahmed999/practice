    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-12 pb-24 md:pb-12">
      <div class="w-full max-w-md shadow-lg bg-card rounded-lg border border-border p-6">
        <div class="text-center mb-6">
          ...
          <p class="text-muted-foreground mt-2">Create your EcoCollect account</p>
        </div>
        <form [formGroup]="registerForm" (ngSubmit)="handleRegister()" class="space-y-5">
          ...
          <app-button type="submit" class="w-full">Create Account</app-button>
          <p class="text-center text-sm text-muted-foreground">
            Already have an account? <a routerLink="/login" class="text-primary hover:underline">{{ t('login') }}</a>
          </p>
        </form>
      </div>
    </div>
